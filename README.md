...
The CSV is only downloaded when `version.txt` changes. A number gets auto blocked when `negative >= 2 × positive` and the "Block numbers with negative rating" toggle is on.
...
Push to GitHub and the `Build APK` workflow produces `app-debug.apk` as an artifact.
